...
payloads = []
rules = open('rules', 'r')
wrongQuery = "-1" # 正确的查询参数 # TODO 这里也需要作为参数提取出来
url = baseUrl + currentQuery
contentLength = len(requests.get(url,verify=False).text)
for line in rules:
    if line.startswith("#"):
        continue
    if line == "\r\n":
    ...
    content1 = requests.get(testUrl1,verify=False).text
    ...
    len1 = len(content1)
    content2 = requests.get(testUrl2,verify=False).text
...
if counter != 0:
    ...
    command = "python bool.py \"" + hack_payload + "\" \"" + baseUrl + "\" \"" + currentQuery + "\""
    log = open('record.log', mode='w')
    print>>log, "Exce : " + command
    log.close()
    os.system(command)
